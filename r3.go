...
func r3(inputPath string, output io.Writer) error {
	...
	resultsCh := make(chan map[string]r3Stats)
	for _, part := range parts {
		go r3ProcessPart(inputPath, part.offset, part.size, resultsCh)
	}

	totals := make(map[string]r3Stats)
	for i := 0; i < len(parts); i++ {
		result := <-resultsCh
		for station, s := range result {
			ts, ok := totals[station]
			if !ok {
				totals[station] = r3Stats{
					min:   s.min,
					max:   s.max,
					sum:   s.sum,
					count: s.count,
...
func r3ProcessPart(inputPath string, fileOffset, fileSize int64, resultsCh chan map[string]r3Stats) {
	...
	stationStats := make(map[string]r3Stats)
